...
module cylinder_outer(height, radius, edges = 6) {
    fudge = 1/cos(180/edges);
    cylinder(h=height,r=radius*fudge,$fn=edges);
}
...
module translate_x(angle) { translate([angle, 0, 0]) children(); }
module translate_y(angle) { translate([0, angle, 0]) children(); }
module translate_z(angle) { translate([0, 0, angle]) children(); }
